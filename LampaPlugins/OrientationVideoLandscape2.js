				...
				if (node.classList && node.classList.contains('player')) {
					...
					if (panel) {
						...
						newDiv.addEventListener('click', function() {
                            ...
                            // Получаем элементы плеера и видео
                            const player = document.body.querySelector('.player');
                            const video = document.body.querySelector('video');

                            if (player) {
                                if (!player.getAttribute('style') || player.style.transform == 'unset') {
                                    // Применяем стили для горизонтальной ориентации плеера
                                    player.style.width           = '100vh';
                                    player.style.height          = '100vw';
                                    player.style.transform       = 'rotate(90deg)';
                                    player.style.transformOrigin = 'bottom left';
                                    player.style.marginTop       = '-100vw';
